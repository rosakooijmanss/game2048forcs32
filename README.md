...
In step 1 of main sliding logic, we changed it in ClaudeAI from a for loop to a list to make our script shorter.
...
Initially, we had the display written very easily at first. Before we added the game loop, we wanted to improve the display so it was easier to use for the user. Therefore, we asked ClaudeAI to enhance our script to improve the display. 

After we added the game loop and played the game, we found out that whenever we pressed one move consecutively without any tiles moving, it would still add new tiles. This would be problematic as it would end the game quickly and that does not happen in the real game. Therefore, we asked ClaudeAI to enhance it. It came up with another if statement that only adds a new tile whenever the game board is changed. 

Even though our game works, it did not look very pretty. Therefore, we asked ClaudeAI to create an interface, which looked very nice. However, the code was very complicated and did not work in the GitHub codespace. We then asked ClaudeAI to create a different web page that worked really well. 
